...
export default function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.agreed) return setError("Please agree to the Terms & Conditions.");
  ...
  return (
    <div className="min-h-screen bg-[#D9D9DB] flex flex-col items-center justify-center px-6">
      ...
      {error && (
        <div className="w-full max-w-lg mb-4 px-4 py-3 bg-red-100 text-red-700 rounded-lg text-sm">
          {error}
        </div>
      )}
      ...
      <form className="w-full max-w-lg space-y-4" onSubmit={handleSubmit}>
          ...
          <label className="flex items-center cursor-pointer p-3 bg-[#D9D9DB] rounded-lg">
            <div className="relative flex items-center justify-center w-8 h-8 rounded-full bg-[#36393F] mr-3">
              <div className="absolute flex items-center justify-center w-5 h-5 rounded-full bg-white shadow-[inset_1px_1px_2px_rgba(0,0,0,0.1),inset_-1px_-1px_2px_rgba(255,255,255,0.7)]">
                ...
                <svg className="h-3 w-3 text-[#36393F] opacity-0 peer-checked:opacity-100 transition-opacity duration-200"
        ...
        <div className="flex justify-center">
          <SubmitButton type="submit" className="w-full max-w-lg" disabled={loading}>
      ...
      <div className="flex flex-col items-center my-8">
        ...
        <div className="flex items-center justify-center gap-9 mb-7">
          <button type="button" className="w-15 h-15 bg-white rounded-full flex items-center justify-center shadow hover:scale-105 transition-transform">
        ...
        <p className="text-sm text-[#545454] font-bold mt-5">
          ...
          <span className="font-extrabold underline text-[#1D1F2A] cursor-pointer hover:text-gray-700"
