    ...
    <div className="min-h-screen bg-[#D9D9DB] flex flex-col">
      ...
      <div className="flex flex-col items-center px-6 md:px-16 lg:px-28 py-6 space-y-6">
        <div className="bg-[#1D1F2A] text-white rounded-2xl p-5 w-full max-w-md md:max-w-lg lg:max-w-2xl h-[190px] shadow-md flex flex-col justify-between">
          ...
          <div>
            <p className="tracking-widest text-lg font-medium mt-6">{cardNumber}</p>
        ...
        <div className="w-full max-w-md md:max-w-lg lg:max-w-2xl space-y-4">
          <div>
            <label className="text-sm font-medium text-[#1D1F2A] mb-1 block">Card Name*</label>
            <TextInput value={cardName} onChange={(e) => setCardName(e.target.value)} placeholder="Enter card name" />
          </div>

          <div>
            <label className="text-sm font-medium text-[#1D1F2A] mb-1 block">Card Number*</label>
            <TextInput value={cardNumber} onChange={(e) => setCardNumber(e.target.value)} placeholder="Enter card number" />
          </div>

          <div className="flex flex-col md:flex-row md:space-x-3 space-y-4 md:space-y-0">
            <div className="flex-1">
              <label className="text-sm font-medium text-[#1D1F2A] mb-1 block">Expiry Date*</label>
              <TextInput value={expiryDate} onChange={(e) => setExpiryDate(e.target.value)} placeholder="MM/YY" />
            </div>
            <div className="flex-1">
              <label className="text-sm font-medium text-[#1D1F2A] mb-1 block">CVV*</label>
              <TextInput value={cvv} onChange={(e) => setCvv(e.target.value)} placeholder="***" />
            </div>
          </div>
        </div>
      </div>
